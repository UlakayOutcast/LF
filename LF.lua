...
SlashCmdList["LF"] = function(cmd)
    if cmd then 
        if string.sub(cmd, 1, 4) == "spam" or string.sub(cmd, 1, 2) == "" then 
...
function LF_OnEvent(event, arg1)
	...
	if event == "CHAT_MSG_CHANNEL" and LF_CONFIG['spamChat']then 
		...
		if arg9 == "LookingForGroup" or arg9 == "world" then 
			if (strfind(str,"lfm",1,true) or strfind(str,"lf.m",1,true) or strfind(str," lf ",1,true) or strfind(string.sub(str, 1, 2),"lf",1,true)) then 
			-- DEFAULT_CHAT_FRAME:AddMessage("\124cff00ff99"..arg2.."\124r".." - ".."\124cFFC2C050"..arg1.."\124r")
				...
				if strfind(str,"gilneas") or strfind(str," gs ") then inst="Gilneas City";end 
